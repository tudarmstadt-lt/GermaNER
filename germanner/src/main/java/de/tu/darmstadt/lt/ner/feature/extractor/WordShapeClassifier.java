/**
 * This code is re-used  from http://plash2.iis.sinica.edu.tw/svn/autoquiz/AutoGrmmarQuiz3/src/edu/stanford/nlp/process/WordShapeClassifier.java
 *
 */
...
/**
 * Provides static methods which map any String to another String indicative of its "word shape" --
 * e.g., whether capitalized, numeric, etc. Different implementations may implement quite different,
 * normally language specific ideas of what word shapes are useful.
 *
...
public class WordShapeClassifier
{
     ...
     *
     * @param name
     *            Shaper name. Known names have patterns along the lines of: dan[12](bio)?(UseLC)?,
    ...
    /**
     * Returns true if the specified word shaper doesn't use known lower case words, even if a list
     * of them is present. This is used for backwards compatibility. It is suggested that new word
     * shape functions are either passed a non-null list of lowercase words or not, depending on
     * whether you want knownLC marking (if it is available in a shaper). This is how chris4 works.
     *
     * @param shape
     *            One of the defined shape constants
     * @return true if the specified word shaper uses known lower case words.
     */
    ...
    /**
     * Specify the String and the int identifying which word shaper to use and this returns the
     * result of using that wordshaper on the String.
     *
     * @param inStr
     *            String to calculate word shpape of
     * @param wordShaper
     ...
     *            Constant for which shaping formula to use
     * @param knownLCWords
     *            A Collection of known lowercase words, which some shapers use to decide the class
     *            of capitalized words. <i>Note: while this code works with any Collection, you
     *            should provide a Set for decent performance.</i> If this parameter is null or
     *            empty, then this option is not used (capitalized words are treated the same,
     *            regardless of whether the lowercased version of the String has been seen).
     * @return The wordshape String
     */
    ...
    /**
     * A fairly basic 5-way classifier, that notes digits, and upper and lower case, mixed, and
     * non-alphanumeric.
     *
     * @param s
    ...
    /**
     * A fine-grained word shape classifier, that equivalence classes lower and upper case and
     * digits, and collapses sequences of the same type, but keeps all punctuation, etc.
     * <p>
     * <i>Note:</i> We treat '_' as a lowercase letter, sort of like many programming languages. We
     * do this because we use '_' joining of tokens in some applications like RTE.
     *
     * @param s
     *            The String whose shape is to be returned
     * @param knownLCWords
     *            If this is non-null and non-empty, mark words whose lower case form is found in
     *            the Collection of known lower case words
    ...
    /**
     * Note: the optimizations in wordShapeChris2 would break if BOUNDARY_SIZE was greater than the
     * shortest greek word, so valid values are: 0, 1, 2, 3.
     */
    private static final int BOUNDARY_SIZE = 2;

    /**
     * This one picks up on Dan2 ideas, but seeks to make less distinctions mid sequence by sorting
     * for long words, but to maintain extra distinctions for short words. It exactly preserves the
     * character shape of the first and last 2 (i.e., BOUNDARY_SIZE) characters and then will record
     * shapes that occur between them (perhaps only if they are different)
     *
     * @param s
     *            The String to find the word shape of
     * @param omitIfInBoundary
     *            If true, character classes present in the first or last two (i.e., BOUNDARY_SIZE)
     *            letters of the word are not also registered as classes that appear in the middle
     *            of the word.
     * @param knownLCWords
     *            If non-null and non-empty, tag with a "k" suffix words that are in this list when
     *            lowercased (representing that the word is "known" as a lowercase word).
    ...
    // sb is maximally of size s.length() + 1, but is usually (much) shorter. The +1 might happen if
    // markKnownLC is true and it applies
    // boundSet is maximally of size BOUNDARY_SIZE * 2 (and is often smaller)
    // seenSet is maximally of size s.length() - BOUNDARY_SIZE * 2, but might often be of size <= 4.
    // But it has no initial size allocation
    // But we want the initial size to be greater than BOUNDARY_SIZE * 2 * (4/3) since the default
    // loadfactor is 3/4.
            ...
            Collection<String> knownLCWords)
    {
        ...
        int beginUpto = 0;
        int endUpto = 0;
        final Set<Character> seenSet = new TreeSet<Character>(); // TreeSet guarantees stable
                                                                 // ordering; has no size parameter
    ...
    private static char chris4equivalenceClass(final char c)
    {
        ...
        if (Character.isDigit(c) || type == Character.LETTER_NUMBER
                ...
                || "ä¸€äºŒä¸‰å››äº”å…­ä¸ƒå…«ä¹åé›¶ã€‡ç™¾åƒä¸‡äº¿å…©â—‹â—¯".indexOf(c) > 0) {
            // include Chinese numbers that are just of unicode type OTHER_LETTER (and a couple of
            // round symbols often used (by mistake?) for zeroes)
            return 'd';
            /*
             * } else if (c == 'ç¬¬') { return 'o'; // detect those Chinese ordinals! } else if (c
             * == 'å¹´' || c == 'æœˆ' || c == 'æ—¥') { // || c == 'å·') { return 'D'; // Chinese
             * date characters.
             */}
        else if (Character.isLowerCase(c)) {
            return 'x';
    ...
    /**
     * This one picks up on Dan2 ideas, but seeks to make less distinctions mid sequence by sorting
     * for long words, but to maintain extra distinctions for short words, by always recording the
     * class of the first and last two characters of the word. Compared to chris2 on which it is
     * based, it uses more Unicode classes, and so collapses things like punctuation more, and might
     * work better with real unicode.
     *
     * @param s
     *            The String to find the word shape of
     * @param omitIfInBoundary
    ...
    /**
     * Returns a fine-grained word shape classifier, that equivalence classes lower and upper case
     * and digits, and collapses sequences of the same type, but keeps all punctuation. This adds an
     * extra recognizer for a greek letter embedded in the String, which is useful for bio.
     */
    ...
    /**
     * List of greek letters for bio. We omit eta, mu, nu, xi, phi, chi, psi. Maybe should omit rho
     * too, but it is used in bio "Rho kinase inhibitor".
     */
    private static final String[] greek = { "alpha", "beta", "gamma", "delta", "epsilon", "zeta",
            "theta", "iota", "kappa", "lambda", "omicron", "rho", "sigma", "tau", "upsilon",
            "omega" };
    private static final Pattern biogreek = Pattern
            .compile(
                    "alpha|beta|gamma|delta|epsilon|zeta|theta|iota|kappa|lambda|omicron|rho|sigma|tau|upsilon|omega",
                    Pattern.CASE_INSENSITIVE);

    /**
     * Somewhat ad-hoc list of only greek letters that bio people use, partly to avoid false
     * positives on short ones.
     *
     * @param s
     *            String to check for Greek
    ...
    /**
     * This one equivalence classes all strings into one of 24 semantically informed classes,
     * somewhat similarly to the function specified in the BBN Nymble NER paper (Bikel et al. 1997).
     * <p>
     * Note that it regards caseless non-Latin letters as lowercase.
     *
     * @param s
     *            String to word class
    ...
    private static String wordShapeChris1(String s)
    {
        int length = s.length();
        if (length == 0) {
            return "SYMBOL"; // unclear if this is sensible, but it's what a length 0 String
                             // becomes....
        }
     ...
     * where <code>name</code> is an argument to <code>lookupShaper</code>. Known names have
     * patterns along the lines of: dan[12](bio)?(UseLC)?, jenny1(useLC)?, chris[1234](useLC)?. If
     * you don't specify a word shape function, you get chris1.
     *
     * @param args
    ...
    public static void main(String[] args)
    {
        ...
        if (args.length == 0) {
            System.out
                    .println("edu.stanford.nlp.process.WordShapeClassifier [-wordShape name] string+");
        }
